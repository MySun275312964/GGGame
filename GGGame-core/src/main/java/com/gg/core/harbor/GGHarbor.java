...
public class GGHarbor {
    ...
    private static void handshake(String host, int port) {
        ...
        MasterRegisterMessage msg =
                MasterRegisterMessage.newBuilder().setService(service).setHost(host).setPort(port).build();
        CountDownLatch waitForHandshake = new CountDownLatch(1);
        StreamObserver<MasterRegisterMessage> request =
